...
export default class InputNameModal extends Phaser.Scene {
    ...
    create() {
        ...
        const buttonImage = this.add.image(width * 0.45, height * 0.58, 'button')
                            .setOrigin(0)
                            .setInteractive()
                            .on('pointerdown', () => { this.showFirstQuestionModal(inputText.text) })
                            .on('pointerover', () => { buttonImage.setTint(0xfadcaa); })
                            .on('pointerout', () => { buttonImage.clearTint(); });

        const buttonText = new TextLabel(this, width * 0.48, height * 0.58, 'Continuar', { fontFamily: 'Comic Sans MS', fontSize: '18px', fill: '#000', align: 'right', padding: 10 })
