...
export default class StoryModal extends Phaser.Scene {
    ...
    showCommandsHelp(width, height) {
        if (this.sys.game.device.os.desktop){
            const labelTextBody = new TextLabel(this, width * 0.05, -height * 0.05, BODY_LABEL, { fontFamily: 'Comic Sans MS', fontSize: '28px', fill: '#000', align: 'right', padding: 10, wordWrap: { width: width / 3  }})
                                  .setOrigin(0.5);
            ...
            const labelTextHelp = new TextLabel(this, width * 0.05, height * 0.20, DESKTOP_LABEL, { fontFamily: 'Comic Sans MS', fontSize: '30px', fill: '#000', align: 'right', padding: 10, wordWrap: { width: width / 3 }})
                                      .setOrigin(0.5);
            const arrowKeys = new UiImage(this, -width * 0.20, height * 0.20, ARROW_KEYS_IMAGE)
                                      .setOrigin(0.5);
    ...
    returnToGame() {
        ...
        this.scene.resume('game-scene', { scene: 'story-modal' });
    }
}